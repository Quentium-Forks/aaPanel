...
class main(dockerBase):
        ...
        for conf in project_conf:
            if conf["key"] != "REMARK" and type(conf["value"]) != list:
                if re.search('\s', conf["value"]):
                    ...
                    return public.returnMsg(False, "{} cannot contain Spaces".format(server_ps))
            if conf["key"] != "VOLUME_PATH" and conf["key"] != "REMARK":
                if conf["value"] == "":
                    server_ps = self.__get_server_ps(project_conf, conf["key"])
                    return public.returnMsg(False, "{} cannot be null!".format(server_ps))
            ...
            if conf["key"].upper() == "PORT":
                if dp.check_socket(conf["value"]):
                    return public.returnMsg(False, "Server port [{}] is occupied, please change to another port!".format(conf['value']))
    ...
    def sync_compose_template(self, server_name):
        ...
        pdata = {
            "name": server_name,
            "remark": "aaPanel Docker Quick Deployment templates only [don't delete them and use them separately to create projects]",
