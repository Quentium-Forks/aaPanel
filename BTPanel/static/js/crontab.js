...
function GetLogs(id) {
	...
	$.post('/crontab?action=GetLogs', data, function (rdata) {
		...
		layer.open({
			...
			closeBtn: 2,
			content:
				'<div class="setchmod bt-form  pb70" style="padding-top: 0;">\
        <pre class="crontab-log" style="overflow: auto; border: 0 none; line-height:23px;padding: 15px; margin: 0;white-space: pre-wrap; height: 405px; background-color: rgb(51,51,51);color:#f1f1f1;border-radius:0;"></pre>\
          ...
          <button type="button" class="btn btn-success btn-sm btn-title" onclick="layer.closeAll()">' +
			...
			success: function () {
				var log_body = rdata.msg === '' ? 'The current log is empty' : rdata.msg,
		...
		if (rdata == []) {
			...
			$.post('/crontab?action=GetDataList', { type: 'sites' }, function (res) {
				...
				for (var i = 0; i < rdata.length; i++) {
					...
					var optName = '';
					if (rdata[i].status != 1)
						s_status = '<span onclick="set_task_status(' + rdata[i].id + ',1)"  class="btClose" style="color:red;cursor:pointer" title="' + lan.crontab.crontab_start + '">' + lan.crontab.disable + '<span style="color:rgb(255, 0, 0);" class="glyphicon glyphicon-pause"></span></span> ';
					...
					cbody +=
						"<tr>\
            <td><input type='checkbox' onclick='checkSelect();' title='" +
						rdata[i].name +
						"' name='id' value='" +
						rdata[i].id +
						"'></td>\
            ...
            <td>\
              <a href="javascript:StartTask(' +
						...
						lan.public.exec +
						'</a> | \
              <a href="javascript:edit_task_info(\'' +
						rdata[i].id +
						"');\" class='btlink'>" +
						lan.files.file_menu_edit +
						'</a> | \
              <a href="javascript:GetLogs(' +
						rdata[i].id +
						");\" class='btlink'>" +
						lan.public.log +
						'</a> | \
              <a href="javascript:planDel(' +
						rdata[i].id +
						" ,'" +
						rdata[i].name.replace('\\', '\\\\').replace("'", "\\'").replace('"', '') +
						"');\" class='btlink'>" +
						lan.public.del +
						'</a>\
            </td>\
...
function edit_task_info(id) {
	...
	$.post('/crontab?action=get_crond_find', { id: id }, function (rdata) {
		...
		obj = {
			...
			sTypeArray: [
				['toShell', lan.crontab.shell],
				['site', lan.crontab.site_bak],
				['database', lan.crontab.db_bak],
				['logs', lan.crontab.log_split],
				['path', lan.crontab.dir_bak],
				['rememory', lan.crontab.mem_release],
				['toUrl', lan.crontab.get_url],
			],
			cycleArray: [
				['day', lan.crontab.daily],
				['day-n', lan.crontab.n_day],
				['hour', lan.crontab.hourly],
				['hour-n', lan.crontab.n_hour],
				['minute-n', lan.crontab.n_min],
				['week', lan.crontab.weekly],
				['month', lan.crontab.monthly],
			],
			weekArray: [
				[1, lan.crontab.TZZ1],
				[2, lan.crontab.TZZ2],
							...
							$.post('/config?action=get_settings2', { type: 'sites' }, function (rdata) {
								if (rdata.user_mail.user_name && rdata.telegram.setup) {
									...
									obj.sBody.messageChannelDom =
										'<li><a role="menuitem" tabindex="-1" href="javascript:;" value="telegram,mail">All</a></li><li><a role="menuitem" tabindex="-1" href="javascript:;" value="telegram">Telegram</a></li><li><a role="menuitem" tabindex="-1" href="javascript:;" value="mail">Email</a></li>';
		...
		obj.create(function () {
          ...
          <div class="setting-con ptb20">\
            <div class="clearfix plan ptb10">\
              <span class="typename c4 pull-left f14 text-right mr20">' +
              ...
              <div class="dropdown stype_list pull-left mr20">\
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="sType">' +
					sTypeDom +
					'</ul>\
              </div>\
            </div>\
            <div class="clearfix plan ptb10">\
              <span class="typename c4 pull-left f14 text-right mr20">' +
					...
					'</span>\
              <div class="planname pull-left"><input type="text" name="name" class="bt-input-text sName_create" value="' +
					obj.from.name +
					'"></div>\
            </div>\
            <div class="clearfix plan ptb10">\
              ...
              <div class="pull-left optional_week">\
                <div class="dropdown week_btn pull-left mr20" style="display:' +
					(obj.from.type == 'week' ? 'block;' : 'none') +
					'">\
                ...
                <div class="plan_hms pull-left mr20 bt-input-text where1_input" style="display:' +
					(obj.from.type == 'day-n' || obj.from.type == 'month' ? 'block;' : 'none') +
					'">\
                  <span>\
                    <input type="number" name="where1" class="where1_create" value="' +
					obj.from.where1 +
					'" maxlength="2" max="23" min="0" />\
                  </span>\
                  <span class="name">' +
					lan.crontab.sun +
					'</span>\
                </div>\
                <div class="plan_hms pull-left mr20 bt-input-text hour_input" style="display:' +
					(obj.from.type == 'day' || obj.from.type == 'day-n' || obj.from.type == 'hour-n' || obj.from.type == 'week' || obj.from.type == 'month' ? 'block;' : 'none') +
					'">\
                  <span>\
                    <input type="number" name="hour" class="hour_create" value="' +
					(obj.from.type == 'hour-n' ? obj.from.where1 : obj.from.hour) +
					'" maxlength="2" max="23" min="0" />\
                  </span>\
                  <span class="name">' +
					lan.crontab.hour1 +
                ...
                <div class="plan_hms pull-left mr20 bt-input-text minute_input">\
                  <span>\
                    <input type="number" name="minute" class="minute_create" value="' +
					(obj.from.type == 'minute-n' ? obj.from.where1 : obj.from.minute) +
            ...
            <div class="clearfix plan ptb10 site_list">\
              ...
              <div style="line-height:34px">\
                <div class="dropdown pull-left mr20 sName_btn" style="display:' +
					(obj.from.sType != 'path' ? 'block;' : 'none') +
					'">\
                  ...
                  <ul class="dropdown-menu" role="menu" aria-labelledby="sName">' +
					sNameDom +
					'</ul>\
                </div>\
                <div style="line-height:34px">\
                  <div class="pull-left" style="margin-right:20px;display:' +
					(obj.from.sType == 'path' ? 'block;' : 'none') +
					'">\
                    <input type="text" name="name" class="bt-input-text sName_create" value="' +
					obj.from.sName +
					'" disabled="disabled">\
                  </div>\
                </div>\
                <div class="textname pull-left mr20">' +
                ...
                <div class="dropdown pull-left mr20 serverDiskToLocal">\
                  ...
                  <ul class="dropdown-menu" role="menu" aria-labelledby="backupTo">' +
					backupsDom +
					'</ul>\
                </div>\
                <div class="textname pull-left mr20">' +
                ...
                <div class="plan_hms pull-left mr20 bt-input-text">\
                  <span>\
                    <input type="number" name="save" class="save_create" value="' +
					obj.from.save +
					'" maxlength="4" max="100" min="1" />\
                  </span>\
                  <span class="name">' +
					lan.crontab.copies +
					'</span>\
                </div>\
                <div class="textname pull-left local-backup-line" style="display: ' +
					(obj.from.backupTo && obj.from.backupTo !== 'localhost' ? 'block' : 'none') +
					';">\
                  <i class="form-checkbox mr5 ' +
					(obj.from.save_local == 1 ? 'active' : '') +
					'"></i>\
                  <input type="checkbox" class="form—checkbox-input hide mr10" name="mod_save_local" ' +
					(obj.from.save_local == 1 ? 'checked' : '') +
					' />\
            ...
            <div class="clearfix plan ptb10 site_list">\
              <span class="typename controls c4 pull-left f14 text-right mr20" style="display:' +
					(obj.from.sType == 'logs' ? 'none' : 'block') +
					';">Backup reminder</span>\
              <div style="line-height:34px" >\
                <div class="dropdown pull-left mr20 sName_btn" id="modNotice" style="display:' +
					(obj.from.sType == 'logs' ? 'none' : 'block') +
					';">\
                  <button class="btn btn-default dropdown-toggle" type="button" id="excode"  data-toggle="dropdown" style="width:180px;">\
					...
					obj.from.notice +
					'">' +
					(obj.from.notice == 1 ? 'Notify on failure' : 'Dont notify') +
					'</b>\
                    <span class="caret"></span>\
                  </button>\
                ...
                <div class="pull-left mr20"  style="display:' +
					(parseInt(obj.from.notice) == 1 && obj.from.sType != 'logs' ? 'block' : 'none') +
					';">notification</div>\
                <div class="dropdown  pull-left mr20"  style="display:' +
					(obj.from.notice == 1 ? 'block' : 'none') +
					';">\
                  <button class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown" style="width:auto;" id="modNotice_channel">\
                    <b val="' +
					obj.from.notice_channel +
					'" id="modNotice_channelValue">' +
					obj.sBody.title +
					'</b>\
                    <span class="caret"></span>\
                  </button>\
            ...
            <div class="clearfix plan ptb10"  style="display:' +
					(obj.from.sType == 'toShell' || obj.from.sType == 'site' || obj.from.sType == 'path' ? 'block;' : 'none') +
					'">\
              <span class="typename controls c4 pull-left f14 text-right mr20">' +
					(obj.from.sType == 'toShell' ? 'Script content' : 'Exclusion rule') +
					'</span>\
              <div style="line-height:22px"><textarea style="line-height:22px" class="txtsjs bt-input-text sBody_create" name="sBody">' +
					obj.from.sBody +
					'</textarea></div>\
            </div>\
            <div class="clearfix plan ptb10" style="display:' +
					(obj.from.sType == 'rememory' ? 'block;' : 'none') +
					'">\
              <span class="typename controls c4 pull-left f14 text-right mr20">' +
					lan.crontab.tips +
					'</span>\
            ...
            <div class="clearfix plan ptb10" style="display:' +
					(obj.from.sType == 'toUrl' ? 'block;' : 'none') +
					'">\
              <span class="typename controls c4 pull-left f14 text-right mr20">' +
					lan.crontab.url_address +
					'</span>\
              <div style="line-height:34px"><input type="text" style="width:400px; height:34px" class="bt-input-text url_create" name="urladdress"  placeholder="' +
					lan.crontab.url_address +
					'" value="' +
					obj.from.urladdress +
					'"></div>\
            ...
            <div class="clearfix plan ptb10">\
              <div class="bt-submit plan-submits " style="margin-left: 141px;">' +
					lan.crontab.edit_save +
					'</div>\
            </div>\
            <div class="clearfix plan ptb10" style="display:' +
					(obj.from.sType == 'webshell' ? 'none' : 'none') +
					'">\
...
function edit_message_channel(type) {
	$.post('/config?action=get_settings2', function (res) {
        ...
        <label style="font-weight: normal;font-size: 14px;margin-left: 6px;display: inline;">dingding</label>\
			...
			tMess =
				'<div class="check_alert" style="margin-right:20px;display: inline-block;">\
        <input type="radio" name="alert" title="Email" value="mail" ' +
				(type == 'mail' ? 'checked' : '') +
				'>\
        <label style="font-weight: normal;font-size: 14px;margin-left: 6px;display: inline;">Email</label>\
      </div>\
	...
	$.post('/crontab?action=GetDataList', data, function (rdata) {
        ...
        <div class="dropdown pull-left mr20">\
          <button class="btn btn-default dropdown-toggle" type="button" id="backdata" data-toggle="dropdown" style="width:auto">\
            <b id="sName" val="' +
				(type === 'logs' ? 'ALL' : rdata.data[0].name) +
				'">' +
				(type === 'logs' ? 'ALL' : rdata.data[0].name + '[' + rdata.data[0].ps + ']') +
				'</b> <span class="caret"></span>\
          </button>\
          <ul class="dropdown-menu" role="menu" aria-labelledby="backdata">\
            <li>\
              <a role="menuitem" tabindex="-1" href="javascript:;" value="ALL">' +
				lan.public.all +
				'</a>\
            </li>\
            ' +
      ...
      ';
        ...
        <div class="info-r" style="display: inline-block;float: left;margin-right: 25px;">\
          <input id="inputPath" class="bt-input-text mr5" type="text" name="path" value="/www/wwwroot/" placeholder="' +
				lan.crontab.dir_bak +
				'" style="width:208px;height:33px;" />\
      ...
      ';
		...
		var sBody =
			sOptBody +
			'\
      <div class="textname pull-left mr20" style="display:' +
			(type === 'logs' ? 'none' : 'inline-block') +
			'">' +
			lan.crontab.backup_to +
			'</div>\
      <div class="dropdown planBackupTo pull-left mr20 serverDiskToLocal" style="display:' +
			(type === 'logs' ? 'none' : 'inline-block') +
			'">\
        ...
        <ul class="dropdown-menu" role="menu" aria-labelledby="excode">\
          <li>\
            <a role="menuitem" tabindex="-1" href="javascript:;" value="localhost">' +
			lan.crontab.disk +
			'</a>\
          </li>\
          ' +
      ...
      <div class="plan_hms pull-left mr20 bt-input-text">\
        <span>\
          <input type="number" name="save" id="save" value="' +
			save_num +
			'" maxlength="4" max="100" min="1" />\
        </span>\
      </div>\
    ...
    ';
		if (type == 'sites' || type == 'path' || type == 'databases') {
          ...
          <p class="clearfix plan">\
            ...
            <div class="dropdown planBackupTo pull-left mr20" style="display:' +
					(type === 'logs' ? 'none' : 'inline-block') +
					'"  id="notice">\
              <button class="btn btn-default dropdown-toggle" type="button" id="excode" data-toggle="dropdown" style="width:180px;">\
        ...
        ';
				if (type == 'sites' || type == 'path') {
					sBody +=
						'\
            <p class="clearfix plan">\
              <div class="textname pull-left mr20" style="width: 120px;text-align: right; font-size: 14px;">' +
						lan.crontab.exclusion_rule +
						'</div>\
              <div class="dropdown planBackupTo pull-left mr20">\
                <span>\
                  <textarea style=" height: 100px;width:300px;line-height:22px;" class="bt-input-text" type="text" name="sBody" id="exclude" placeholder="' +
						lan.crontab.exclusion_rule_tips +
						'\ndata/config.php\nstatic/upload\n *.log\n"></textarea>\
                </span>\
              </div>\
            </p>\
...
function toWeek() {
	var mBody =
		...
		'</b> <span class="caret"></span>\
            ...
            <ul class="dropdown-menu" role="menu" aria-labelledby="excode">\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="1">' +
		lan.crontab.TZZ1 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="2">' +
		lan.crontab.TZZ2 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="3">' +
		lan.crontab.TZZ3 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="4">' +
		lan.crontab.TZZ4 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="5">' +
		lan.crontab.TZZ5 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="6">' +
		lan.crontab.TZZ6 +
		'</a></li>\
            <li><a role="menuitem" tabindex="-1" href="javascript:;" value="0">' +
		lan.crontab.TZZ7 +
		'</a></li>\
            </ul>\
          </div>';
...
function toShell() {
	...
	if ($("b[val='toShell']").text() === 'synchronised time') {
		...
		shell_body =
			'echo "|-Trying to synchronize time from 0.pool.bt.cn..";\n\
