...
Download_Src(){
	...
	unzip -o lua-nginx-module-master.zip
	mv lua-nginx-module-master lua_nginx_module
	rm -f lua-nginx-module-master.zip

	unzip -o ngx_cache_purge-master.zip
	mv ngx_cache_purge-master ngx_cache_purge
	rm -f ngx_cache_purge-master.zip

	unzip -o nginx-sticky-module-ng-master.zip
	mv nginx-sticky-module-ng-master ngx_sticky_module
	rm -f nginx-sticky-module-ng-master.zip

	unzip -o nginx-http-concat-master.zip
	mv nginx-http-concat-master ngx_http_concat
	rm -f nginx-http-concat-master.zip

	unzip -o ngx_devel_kit-master.zip
	mv ngx_devel_kit-master ngx_devel_kit
	rm -f ngx_devel_kit-master.zip	

	unzip -o nginx-dav-ext-module-master.zip
	mv nginx-dav-ext-module-master ngx_dav_ext_module
	rm -f nginx-dav-ext-module-master.zip
...
Install_Configure(){
	...
	if [ "${WebDav_NGINX}" -ge "114" ] && [ "${WebDav_NGINX}" != "181" ];then	
		ENABLE_WEBDAV="--with-http_dav_module --add-module=${Setup_Path}/src/ngx_dav_ext_module"
	fi
	...
	./configure --user=www --group=www --prefix=${Setup_Path} ${ENABLE_LUA} --add-module=${Setup_Path}/src/ngx_cache_purge --add-module=${Setup_Path}/src/ngx_sticky_module --with-openssl=${Setup_Path}/src/openssl --with-pcre=pcre-${pcre_version} ${ENABLE_HTTP2} --with-http_stub_status_module --with-http_ssl_module --with-http_image_filter_module --with-http_gzip_static_module --with-http_gunzip_module --with-ipv6 --with-http_sub_module --with-http_flv_module --with-http_addition_module --with-http_realip_module --with-http_mp4_module --with-ld-opt="-Wl,-E" --with-cc-opt="-Wno-error" ${jemallocLD} ${ENABLE_WEBDAV} ${ENABLE_NGX_PAGESPEED} ${ADD_EXTENSION} ${i_make_args}
	make -j${cpuCore}
}
Install_Nginx(){
	make install
